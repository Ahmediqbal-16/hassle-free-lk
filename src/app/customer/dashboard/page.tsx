...
import { PlusCircle, ClipboardList, LogOut, ChevronRight, Settings } from "lucide-react";
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: taskData } = await supabase
        .from("tasks").select("*").eq("customer_id", user.id).order("created_at", { ascending: false });
      setTasks(taskData || []);
      setLoading(false);
    }
    load();
      ...
      <header className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-gray-400 hover:text-red-500 text-sm transition-colors p-2"
          >
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-3xl p-7 text-white shadow-xl shadow-blue-200">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-blue-200 text-sm font-medium">{t('manageTasksDesc')}</p>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: t('tasksTotal'), value: tasks.length, color: "text-gray-900" },
            { label: t('activeTasks'), value: activeTasks.length, color: "text-blue-600" },
            { label: t('completed'), value: completedTasks.length, color: "text-green-600" },
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <div>
              <div className="font-bold">{t('postTask')}</div>
              <div className="text-blue-200 text-xs">Find help fast</div>
            </div>
          </Link>
          <Link
            href="/customer/tasks"
          ...
          >
            ...
            <div>
              <div className="font-bold text-gray-900">{t('myTasks')}</div>
              <div className="text-gray-400 text-xs">
                {tasks.length} {tasks.length !== 1 ? t('tasksTotal') : t('taskTotal')}
              </div>
            </div>
          </Link>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-900">{t('recentTasks')}</h2>
            {tasks.length > 0 && (
          ...
          {tasks.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-3xl border border-gray-100 shadow-sm">
              ...
              <p className="text-gray-500 mb-5 font-medium">{t('noTasksYet')}</p>
              <Link
                href="/customer/post-task"
              ...
              {tasks.slice(0, 5).map((task) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-gray-400 text-sm mt-0.5 capitalize">{task.category} · {task.location}</div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0 ml-4">
                    <div className="text-right hidden sm:block">
                      <div className="font-bold text-gray-900">LKR {task.budget.toLocaleString()}</div>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${statusColors[task.status]}`}>
