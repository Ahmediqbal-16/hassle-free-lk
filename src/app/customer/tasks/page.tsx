...
const statusLabel: Record<string, string> = {
  ...
  confirmed: "Confirmed — provider assigned",
    ...
    <div className="min-h-screen bg-gray-50 pb-20 sm:pb-0">
      <header className="bg-white border-b border-gray-100 px-4 sm:px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        ...
        >
          ...
          <span className="hidden sm:block">Book Service</span>
      ...
      <div className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3 flex gap-2 overflow-x-auto scrollbar-hide">
        {filters.map((f) => (
          ...
          >
            {f === "All" ? `All (${bookings.length})` : `${f.replace("_", " ").replace(/\b\w/g, c => c.toUpperCase())} (${bookings.filter(b => b.status === f).length})`}
          </button>
        ))}
      </div>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
        {filtered.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500 mb-6 font-medium">No bookings yet</p>
            <Link
              href="/customer/post-task"
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-gray-400 mt-1">{statusLabel[booking.status]}</div>
                    </div>
                  </div>
                  <div className="shrink-0 flex flex-col items-end gap-2">
                    <div className="font-black text-gray-900">LKR {booking.package?.price.toLocaleString()}</div>
                    <div className="text-xs text-gray-400 capitalize">{booking.payment_method === "cash" ? "💵 Cash" : "💳 Online"}</div>
