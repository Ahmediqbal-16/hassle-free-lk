  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 sm:pb-0">
      <header className="bg-white border-b border-gray-100 px-4 sm:px-6 py-4 flex items-center gap-3 sticky top-0 z-40">
        ...
        <span className="ml-auto text-sm text-gray-400">{tasks.length} {t('tasksTotal')}</span>
      </header>
      ...
      <div className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3 flex gap-2 overflow-x-auto scrollbar-hide">
        {filters.map((f) => (
          ...
          >
            {f === "All" ? `All (${tasks.length})` : `${f.replace("_", " ").replace(/\b\w/g, c => c.toUpperCase())} (${tasks.filter(t => t.status === f).length})`}
          </button>
        ))}
      </div>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
        {filtered.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500 font-medium">{t('noTasksAccepted')}</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="shrink-0 flex flex-col items-end gap-2">
                    ...
                    {task.status === "completed" && (
                      <div className="text-green-600 text-xs">Earned ~LKR {Math.round(task.budget * 0.85).toLocaleString()}</div>
                    )}
